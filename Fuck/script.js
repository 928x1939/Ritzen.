...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function typeDescription() {
        const descriptions = [
            "Just a guy who codes and listens to music",
            "I Love Playing Minecraft And Learning Chess :hearts:"
        ];
    ...
    function initBackgroundCycling() {
        const backgrounds = [
            { src: 'background1.mp4', type: 'video' },
            { src: 'background2.png', type: 'image' },
            { src: 'background3.png', type: 'image' }
        ];

        function setBackground(bgObj) {
            if (bgObj.type === 'video') {
                bgVideo.src = bgObj.src;
                bgVideo.classList.add('active');
                bgImage.classList.remove('active');
            } else {
                bgImage.src = bgObj.src;
        ...
        const cycle3 = document.getElementById('cycle3');
        
        if (cycle1) cycle1.addEventListener('click', () => setBackground(backgrounds[0]));
        if (cycle2) cycle2.addEventListener('click', () => setBackground(backgrounds[1]));
        if (cycle3) cycle3.addEventListener('click', () => setBackground(backgrounds[2]));
    }
    ...
    function setupBackground() {
        // Initialize with background1.mp4
        currentBg = 'background1.mp4';
        
        bgVideo.src = 'background1.mp4';
    ...
    // ========================================
    // SCROLLING TITLE ANIMATION - COOLER VERSION
    // ========================================
    function scrollTitle() {
        ...
        function updateTitle() {
            ...
            // Scroll effect
            if (scrollOffset >= currentText.length) {
                direction = -1;
            ...
            const scrolledText = currentText.substring(Math.max(0, scrollOffset)) + ' | ' + currentText.substring(0, Math.max(0, scrollOffset));
    ...
    // ========================================
    // GERMAN TIME DISPLAY (CET/CEST)
    // ========================================
    function updateGermanTime() {
        const germanTimeElement = document.getElementById('gmtTime');
        if (germanTimeElement) {
            const now = new Date();
            ...
            germanTimeElement.textContent = `${timeString} CET`;
        }
    }
    ...
    scrollTitle();
    typeStartText();
    updateGermanTime(); // Initial call to show time immediately
    setInterval(updateGermanTime, 1000); // Update time every second
    ...
    // Litecoin button removed

    if (discordBtn) {
        discordBtn.addEventListener('click', function (e) {
            e.preventDefault();
            // Discord button now scrolls to bio card
            const bioCard = document.querySelector('.bio-card');
            if (bioCard) {
                bioCard.scrollIntoView({ behavior: 'smooth' });
                // Or open Discord profile if you want
                // window.open('https://discord.com/users/1013213758446895215', '_blank');
            }
        });
    }
    ...
    const DISCORD_USER_ID = '1013213758446895215';
    const statusIndicator = document.getElementById('statusIndicator');
    const activityContent = document.getElementById('activityContent');

    async function fetchDiscordActivity() {
        try {
            const response = await fetch(`https://api.lanyard.rest/v1/users/${1013213758446895215}`);
        ...
        activityContent.innerHTML = `
            <div class="setup-instructions">
                ...
                <div class="setup-steps">
                    ...
                    2. Join https://discord.gg/kvchgKwhV4<br>
